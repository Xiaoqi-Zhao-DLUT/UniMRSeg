...
from medical.trainer_ssl2 import Trainer, Validator
...
from medical.model.UNet3D_ASPP import UNet3D_3DASPP_ssl2_new

parser = argparse.ArgumentParser(description='UniMRseg for MRI-based Segmentation')
parser.add_argument('--model_name', default="UniMRSeg", help='the model will be trained')
...
def nt_xent_loss(out_1, out_2, temperature):
    ...
    pos_mask[0, 1] = pos_mask[1, 0] = True  # (A, A')
    pos_mask[2, 3] = pos_mask[3, 2] = True  # (B, B')

    neg_mask = ~torch.eye(n_samples, device=sim.device).bool()  
    neg_mask = neg_mask & ~pos_mask  


    pos = sim[pos_mask].view(n_samples)  

    neg = sim[neg_mask].view(n_samples, -1) 
...
def main_worker(gpu, args):
    ...
    if args.checkpoint is not None:
        ...
        if 'outc.conv.weight' in new_state_dict:
            new_state_dict.pop('outc.conv.weight')
        if 'outc.conv.bias' in new_state_dict:
            new_state_dict.pop('outc.conv.bias')

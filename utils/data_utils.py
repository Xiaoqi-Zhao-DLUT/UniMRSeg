...
def datafold_read(datalist,
    ...
    test= []
    for d in json_data:
        if 'fold' in d and d['fold'] == 0:
            test.append(d)
        elif 'fold' in d and d['fold'] == 1:
            val.append(d)
        else:
            tr.append(d)
###当前json和原始论文report的val/test有区别。
    return tr, val, test
...
def get_loader(args):
    ...
    train_files, validation_files, test_files = datafold_read(datalist=datalist_json, basedir=data_dir, fold=args.fold)
